...
let shopModule = (function () {
    ...
    let queryData = function queryData() {
        ...
        xhr.open('get', 'json/product1.json', false);
    ...
    let handle = function handle() {
        ...
        Array.from(navList).forEach((item) => {
            ...
            item.onclick = function () {
                ...
                this.flag *= -1;
                //在结构中把每一个li排序的标识字段设置为自定义属性,点击的时候获取它的排序字段 pai = 'price'/'time'/'hot'
                let pai = this.getAttribute('data-pai');
                data.sort((a, b) => {
                    //只有字符串才有replace方法,我们把获取结果都先变为字符串
    ...
    return {
        //模块的入口,在这里能够按照该有的顺序,一次实现整个模块的相关功能
        init() {
            queryData();
            render();
            handle();
