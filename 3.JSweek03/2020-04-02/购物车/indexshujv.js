let shopMoudle = (function () {
    ...
    let render = function render() {
        ...
        data.forEach(item => {
            ...
            //循环数据中的每一项,计算总购买数量和总价格以及把所有单价存到数组中
            counts += count;
            total += count * price,
            ...
            str += `<li>
                ...
                <span>
                    单价：<strong >${price}元 </strong> 小计：<strong class="xiaoji">${count * price}元</strong>
                </span>
            </li>`
        })
        str += `</ul>`;
        str += `<div class="info">
            ...
            <br />
            <br />
            <span>其中最贵的商品单价是：<em>${Math.max.apply(null, priceArr)}</em>元</span>
        </div>`
        box.innerHTML = str;
    ...
    let handle = function handle() {
        ...
        btns.forEach(item => {
            ...
            item.onclick = function () {
                //每一次点击,我们需要明确出两个值,加还是减?  哪一行(对应那个数据)?
                // 自定义属性的方式：在循环绑定的时候，就给每一个I记录好
                // 1. 加还是减
                // 2. 对应数据中的哪一项（数据中每一项ID）
                // 当点击的时候
                // 只需要从当前点击的这个I中，获取标识值，就啥都知道了
                    let btn = this.getAttribute('data-btn'),
                        id = this.getAttribute('data-id');
                        id = parseInt(id);//getAttribute 获取的结果都是字符串
                        data = data.map(cur=>{
                            console.log(cur);
